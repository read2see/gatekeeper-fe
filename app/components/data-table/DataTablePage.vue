...
const showCreateAction = computed(() => {
  ...
  if (props.config.createAction.requiredPermission) {
    return access.can(props.config.createAction.requiredPermission, props.config.organizationId)
  }
...
const emptyState = computed(() => ({
  title: props.config.emptyState?.title ?? 'Nothing here yet',
  description: props.config.emptyState?.description ?? 'Get started by creating your first item.',
  icon: props.config.emptyState?.icon ?? 'i-lucide-inbox'
}))
